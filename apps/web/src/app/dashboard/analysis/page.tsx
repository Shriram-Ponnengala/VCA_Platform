...
export default function AnalysisBoardPage() {
    ...
    fetch('/api/auth/token', { credentials: 'include' })
      ...
      .then(data => {
        ...
        try {
          ...
          console.log('Current logged-in user role (analysis):', payload.role);
          setUserRole(payload.role);
          setRoomId(`analysis-${payload.id || payload.userId || Date.now()}`);
        } catch (e) {
          console.error('Error decoding token role:', e);
          setRoomId(`analysis-${Date.now()}`);
        }
  ...
  } = useChessRoom(roomId || 'uninitialized', token, { enabled: !!token && !!roomId });
