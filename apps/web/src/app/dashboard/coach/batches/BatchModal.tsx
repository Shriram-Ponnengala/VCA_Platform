...
interface BatchModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (batchData: Omit<Batch, 'id' | 'students' | 'history' | 'status' | 'studentDetails'>) => void;
...
const calculateEndTime = (startTime: string, durationStr: string): string => {
  if (!startTime) return '';
  const [sH, sM] = startTime.split(':').map(Number);
  const totalMinutes = sH * 60 + sM + parseInt(durationStr);
    ...
    if (isOpen) {
      ...
      if (initialData) {
        setFormData({
          ...
          duration: calculateDuration(initialData.startTime || '', initialData.endTime || '')
        });
      } else {
        setFormData({
          name: '',
