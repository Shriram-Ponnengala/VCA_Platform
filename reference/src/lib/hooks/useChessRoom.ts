  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        ...
        try {
          const resp = await fetch('/api/socket', { signal: controller.signal });
          clearTimeout(timeoutId);
          if (!resp.ok && resp.status !== 400) {
            console.warn('[ChessRoom] API Socket wake-up returned status:', resp.status);
          }
        } catch (e: any) {
          if (e.name === 'AbortError') {
            console.warn('[ChessRoom] API Socket wake-up timed out, proceeding anyway');
          } else {
            console.warn('[ChessRoom] API Socket wake-up error:', e.message);
          }
        }
        ...
        console.error('[ChessRoom] Init catch error:', err);
      }
    };
